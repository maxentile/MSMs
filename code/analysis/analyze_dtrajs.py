...
def plot_traj_length_distribution(dtrajs,model_name='',bins=20):
   ''' Create a histogram of trajectory lengths. '''
   plt.hist([len(x) for x in dtrajs],bins=20);
   plt.xlabel('Trajectory length (snapshots)')
   plt.ylabel('Number ')
   plt.title('Trajectory length distribution: {0}'.format(model_name))
   plt.close()
...
def analyze_dtrajs(filename,
   ...
   plot_traj_length_distribution(raw_dtrajs,model_name=model_name)
...
if __name__=='__main__':
   ...
   try:
     ...
     filename = '../results/src_11401_2000_dtrajs.npz'
